...
const isNormalUser = (ctx) => {
  return !!ctx.session.isNormalUser;
};
...
router.get("/apps", isAuthenticated, async (ctx) => {
  ...
  return await ctx.render("apps/normal-dashboard", {
    apps,
  });
});
...
router.get("/apps/:appName", isAuthenticated, async (ctx) => {
  ...
  if (app) {
    ...
    return await ctx.render("apps/normal-app", {
      app,
      logs: {
        stdout,
        stderr,
